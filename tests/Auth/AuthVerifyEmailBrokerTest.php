...
use Mockery as m;
use LaravelVerifyEmails\Contracts\Auth\VerifyEmailBroker;
...
class AuthVerifyEmailBrokerTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $mocks = $this->getMocks();
        $broker = $this->getMock('LaravelVerifyEmails\Auth\VerifyEmails\VerifyEmailBroker', ['emailVerificationLink'], array_values($mocks));
        $user = m::mock('LaravelVerifyEmails\Contracts\Auth\CanVerifyEmail');
    ...
    {
        $broker = $this->getMock('LaravelVerifyEmails\Auth\VerifyEmails\VerifyEmailBroker', ['validateVerification', 'getToken'], array_values($mocks = $this->getMocks()));
        $user = m::mock('LaravelVerifyEmails\Contracts\Auth\CanVerifyEmail');
    ...
    {
        return new \LaravelVerifyEmails\Auth\VerifyEmails\VerifyEmailBroker($mocks['tokens'], $mocks['mailer'], $mocks['view']);
    }

    protected function getMocks()
    {
        $mocks = [
            'tokens' => m::mock('LaravelVerifyEmails\Auth\VerifyEmails\TokenRepositoryInterface'),
